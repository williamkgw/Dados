...
def main():
    ...
    if args.file == 'scrape':
        if args.mode == 'webscraping':
            ...
            logging.info(f"Started to scrape on the following companies {emps}")
        ...
        if args.mode == 'import_icg':
            ...
            logging.info(f"Started to generate imports on the following companies {emps}")
            ...
            logging.info(f"Started to triple check on the following companies {emps}")
            ...
            logging.info(f"Started to generate mapping item on the following companies {emps}")
            ...
            logging.info(f"Started to generate new_mapping_sales on the following companies {emps}")
            ...
            logging.info(f"Started to generate new_mapping_clients on the following companies {emps}")
            ...
            logging.info(f"Started to generate the correct_new_mapping_sales on the following companies {emps}")
            ...
            logging.info(f"Started to generate the data_analysis on the following companies {emps}")
        ...
        if args.mode == 'mapping_clientes':
            ...
            logging.info(f"Started to get the past mapping_clients on the following companies {emps}")
            src.support.copy_mapping_clientes(emps)

        elif args.mode == 'mapping_item':
            config = ConfigLoad('end', 'null')
            emps = dataframe.is_not_done_carga(config.input_dir.cargas.control_flow, 'mapping_item')
            logging.info(f"Started to get the past mapping_item on the following companies {emps}")
            ...
            logging.info(f"Started to get the past mapping_sales on the following companies {emps}")
            ...
            logging.info(f"Started to get the past import template on the following companies {emps}")
            ...
            logging.info(f"Started to post the files on the ftp directory on the following companies {emps}")
            src.support.copy_ftp_dir(emps, config.input_dir.results)

        elif args.mode == 'new_mapping_clientes_all':
            config = ConfigLoad('end', 'null')
            emps = dataframe.is_not_done_carga(config.input_dir.cargas.control_flow, 'new_mapping_cliente')
            logging.info(f"Started to get the aggregated new_mapping_clientes on the following companies {emps}")
            src.support.copy_new_mapping_clientes_all(emps, config.input_dir)

        elif args.mode == 'mapping_item_all':
            config = ConfigLoad('end', 'null')
            emps = dataframe.is_not_done_carga(config.input_dir.cargas.control_flow, 'mapping_item')
            logging.info(f"Started to get the aggregated mapping_item on the following companies {emps}")
            src.support.copy_mapping_item_all(emps, config.input_dir)

        elif args.mode == 'new_mapping_vendas_all':
            config = ConfigLoad('end', 'null')
            emps = dataframe.is_not_done_carga(config.input_dir.cargas.control_flow, 'new_mapping')
            logging.info(f"Started to get the aggregated new_mapping_sales on the following companies {emps}")
            src.support.copy_new_mapping_vendas_all(emps, config.input_dir)

        elif args.mode == 'new_mapping_clientes_all_to_company_dir':
            config = ConfigLoad('end', 'null')
            logging.info(f"Started to post the segregated new_mapping_clients from the directory {config.input_dir}")
            src.support.copy_new_mapping_clientes_all_to_company_dir(config.input_dir)

        elif args.mode == 'mapping_item_all_to_company_dir':
            config = ConfigLoad('end', 'null')
            logging.info(f"Started to post the segregated mapping_item from the directory {config.input_dir}")
            src.support.copy_mapping_item_all_to_company_dir(config.input_dir)

        elif args.mode == 'new_mapping_vendas_all_to_company_dir':
            config = ConfigLoad('end', 'null')
            logging.info(f"Started to post the segregated new_mapping_sales from the directory {config.input_dir}")
            ...
            config = ConfigLoad('end', 'null')
            logging.info(f"Started to post the segregated correct_new_mapping_sales from the directory {config.input_dir}")
